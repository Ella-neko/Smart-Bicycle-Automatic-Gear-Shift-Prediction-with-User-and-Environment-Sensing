...
#----------------------------------參數調整--------------------------------------------
param_test1 = {
    'min_child_samples':[i for i in range(1,20)],
...
gsearch1 = GridSearchCV(estimator = lgb.LGBMClassifier(objective="multiclass",max_depth=1,min_child_samples=1),
...
'''
z=0
clf = xgb.XGBClassifier(n_jobs=-1,n_estimators =1,tree_method='gpu_hist',predictor='gpu_predictor')
for i in range(50):
    print(i)
    if(z==0):
        clf.fit(con_train, y_train,eval_metric="auc")
...
#-------------------------------------------------catboost-----------------------------------------------#good2
#cat_features = [0]
#train_dataset = Pool(data=con_train,label=y_train)
#eval_dataset = Pool(data=con_test,
 #                   label=y_test)
...
for i in range(65):
    ...
    if(z==0):
        ...
        #clf2 = pickle.loads(s)
        #clf2.n_estimators += (1)
        clf2.fit(con_train, y_train,init_model=clf2)
    #clf.save_model('D:/XGB/model')
    #z=clf.save_raw()
    #y_=clf.predict(con_train)
    con_train[1:,-1]=clf2.predict(con_train)[:-1,0]
...
clf1 = lgb.LGBMClassifier(objective="multiclass")
#clf1 = xgb.XGBClassifier(n_jobs=-1,n_estimators =1000)
#clf1=RandomForestClassifier(n_estimators=70,min_samples_leaf = 6)

clf2 = lgb.LGBMClassifier(objective="multiclass",n_estimators=200,max_depth=3,min_child_samples=5)

clf1.fit(con_train,y_train)
#y_val=clf1.predict(con_test[0:test_size])
...
for i in range(50):
    ...
    if(z==0):
        ...
        #clf2 = pickle.loads(s)
        #clf2.n_estimators += (1)
        clf2.fit(con_train, y_train,init_model=(clf2.get_params))
...
#np.savetxt('Result/school/LGBM.csv',y_val, delimiter=',')
